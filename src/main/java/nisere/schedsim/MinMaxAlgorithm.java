    	...
    	while (isNotScheduled) {
	    	...
	    	for (Cloudlet cloudlet : cloudletList) {
				...
				for (Vm vm : vmList) {
					...
					if (minC == -1 || minC > workload[vm.getId()] + cloudlet.getCloudletLength()/vm.getMips()) {
						minC = workload[vm.getId()] + cloudlet.getCloudletLength()/vm.getMips();
						minCE = cloudlet.getCloudletLength()/vm.getMips();
						minCloudlet = cloudlet;
						minVmId = vm.getId();
					}
					if (minE == -1 || minE > cloudlet.getCloudletLength()/vm.getMips()) {
						minE = cloudlet.getCloudletLength()/vm.getMips();
					}
				}
				////find max of Kxj = Cxj/Ehj, where Cxj = min of Cij and Ehj = min of Eij found above
				//find max of Kxj = Exj/Ehj, where Cxj = min of Cij and Exj = the corresponding exec time of Cxj and Ehj = min of Eij found above
				if (minCE >= 0 && minE >= 0 
