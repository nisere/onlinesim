    	...
    	while (isNotScheduled) {
	    	...
	    	for (Cloudlet cloudlet : cloudletList) {
				...
				for (Vm vm : vmList) {
					...
					double cij = workload[vm.getId()] + cloudlet.getCloudletLength()/vm.getMips();
				...
				//find max of Sufferxj = Ckj - Cxj, where Cxj = first min of Cij and Ckj = second min of Cij found above
				if (secondMin < 0) {
					secondMin = firstMin;
				}
				if (firstMin >= 0
